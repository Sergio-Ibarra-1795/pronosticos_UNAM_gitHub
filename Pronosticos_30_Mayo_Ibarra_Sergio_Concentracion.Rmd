---
title: "Pronosticos_30_Mayo_Ibarra_Sergio_Concentracion"
author: "Sergibar"
date: "2023-05-30"
output: html_document
---

## UNAM Maestría en Ingeniería de Sistemas 
## Pronósticos 
## Dr. Jair Morales Camarena 
## Exmámen Final
## Alumno: Ibarra Ramírez Sergio 


Importación de librerias necesarias para trabajar 
```{r}


library(tidyverse)      # data manipulation and visualization
library(lubridate)      # easily work with dates and times
library(fpp2)           # working with time series data
library(zoo)
library(ggplot2)
library(plyr)
library(dplyr)
library(knitr)
library(TTR)
library(hydroGOF)


```


# Concentracion Carbono 

### Importación de los datos de nuestra serie que se va a llamar datos_original


```{r}
library(readxl)


# Read the Excel file
concentracion_datos_original <- read_excel('concentracionCO.xlsx')

# View the imported data
print(concentracion_datos_original)


```


Nos aseguramos que la data se haya importado correctamente
```{r}

summary(concentracion_datos_original)
typeof(concentracion_datos_original)
dim(concentracion_datos_original)
```



#### Convertimos los concentracion_datos_original a Serie de tiempo 

Al observar la data se determina que existe una especie de estacionalidad que se repite cada 12 meses 

```{r}

concentracion_datos_original_ts<- ts(concentracion_datos_original$CONCEN, frequency = 12)
head(concentracion_datos_original_ts)
typeof(concentracion_datos_original_ts)

```


Graficamos la serie del tiempo
```{r}
plot(concentracion_datos_original_ts, col = "violet", main = "Colorado Data_original")

```



### Vamos a descomponer la data usando el metodo decompose para entender sus compoentes de tren, seasonalidad y aleatorio

```{r}
concentracion_datos_original_ts_decompose <- decompose(concentracion_datos_original_ts)
summary(concentracion_datos_original_ts_decompose)
```

Vamos a plotear la desmposición 

```{r}
plot(concentracion_datos_original_ts_decompose)
```

En general para los componentes de trend, estacionalidad y randomnes de nuestra variable Y se observa: 

- Trend: Una clara tendencia en aumento 

- Estacionalidad: Se observa un componente repetitivo ciclico cada 12 meses

- Random: Se observa una especie de ruido blanco, con media cero y una sd de alrededor de 0.4




### Analicemos los valores de la variable y 

```{r}
hist(concentracion_datos_original_ts)
```



```{r}
summary(concentracion_datos_original_ts)
```


Se observa que la gran mayoria de los valores de la variable y están en el intervalo de [335,345] y que la variable puede seguir una distribución más o menos normal 



#### Separamos el concentracion_datos_original_ts en data sets train y test

Se tienen 600 datos mensuales, por lo que el train serpa de 576 datos, para que el test sea de 24 datos o 2 años 

```{r}

# Split the data into training and testing sets


# Subset the data to keep rows from the beginning until train_end and all columns
concentracion_datos_original_ts_train <- concentracion_datos_original_ts[1:144]
head(concentracion_datos_original_ts_train)
tail(concentracion_datos_original_ts_train)


```



```{r}
# Subset the data to keep rows from the beginning until train_end and all columns


concentracion_datos_original_ts_test <- concentracion_datos_original_ts[145:154]
head(concentracion_datos_original_ts_test)
tail(concentracion_datos_original_ts_test)


```


#### Apliquemos un modelo de Simple Moving Average (SMA) a los  datos_original

```{r}

# Load the TTR package
library(TTR)

concentracion_datos_original_ts_SMA <- SMA(concentracion_datos_original_ts, n=3)


concentracion_datos_original_ts_SMA_pc <- SMA(concentracion_datos_original_ts_train, n=3)



## datos_original_ts_SMA <- ma(datos_original_ts, oder=2, center=TRUE)
```



```{r}
plot(concentracion_datos_original_ts_SMA, col = "green", main = "Datos_original con SMA ")
lines(concentracion_datos_original_ts, col="violet")
legend("bottomright", legend = c("Datos_original con SMA", "Datos_original"), col = c("green", "violet"), lty = 1)
```


#### Calculo de predección con Simple Moving Average (SMA) de datos_original

```{r}
library(forecast)

forecast_concentracion_datos_original_ts_SMA<-forecast(concentracion_datos_original_ts_SMA_pc, h=9)
forecast_concentracion_datos_original_ts_SMA
plot(forecast_concentracion_datos_original_ts_SMA)
```

#### Calculo de errror con Simple Moving Average (SMA) de datos_original

```{r}
library(forecast)

accuracy(forecast_concentracion_datos_original_ts_SMA)
```

Con el método de Simple Moving Average (SMA) aplicado a los datos de concentracion, se obtuvo un MAPE de ~ 0.16 en el forecast de 9 meses de datos de test con 144 meses de datos de entrenamiento



####Apliquemos un modelo de Weighted Moving Average (WMA)a los datos_original

```{r}

# Load the TTR package
library(TTR)

concentracion_datos_original_ts_WMA <- WMA(concentracion_datos_original_ts, n = 3, wts = c(0.333, 0.333, 0.333))

concentracion_datos_original_ts_WMA_pc <- WMA(concentracion_datos_original_ts_train, n = 3, wts = c(0.333, 0.333, 0.333))
```



```{r}
plot(concentracion_datos_original_ts_WMA, col = "blue", main = "Datos_original con WMA ")
lines(concentracion_datos_original_ts, col="violet")
legend("bottomright", legend = c("Datos_original con WMA", "Datos_original"), col = c("blue", "violet"), lty = 1)
```




#### Calculo de predección con Weighted Moving Average (WMA) de datos_original

```{r}
library(forecast)

forecast_concentracion_datos_original_ts_WMA<-forecast(concentracion_datos_original_ts_WMA_pc, h=9)
forecast_concentracion_datos_original_ts_WMA
plot(forecast_concentracion_datos_original_ts_WMA)
```

#### Calculo de errror con Weighted Moving Average (WMA) de datos_original

```{r}
library(forecast)

accuracy(forecast_concentracion_datos_original_ts_WMA)
```


Con el método de Wighted Moving Average (WMA) aplicado a los datos de colorado, se obtuvo un MAPE de ~ 0.16 en el forecast de 9 meses de datos de test con 144 meses de datos de entrenamiento



## Ahora intentemos ajustar un 'modelo lineal generalizado simple' a los datos_original

Para ello vamos a construir una especie de "Variables dummy" de los meses del año 

```{r}
tend <- c(1:153)

dim(tend)

mes=c("ene","feb","mar","abr","may","jun","jul","ago","sep","oct","nov","dic")

rep_mes <- rep(mes,13)



concentracion_datos_original_ts_train_frame <- data.frame(concentracion_datos_original_ts_train,rep_mes[1:144],tend[1:144])
head(concentracion_datos_original_ts_train_frame)
tail(concentracion_datos_original_ts_train_frame)
dim(concentracion_datos_original_ts_train_frame)

```

```{r}
concentracion_datos_original_ts_train_frame$rep_mes <-concentracion_datos_original_ts_train_frame$rep_mes.1.144.
concentracion_datos_original_ts_train_frame$rep_mes.1.144. <- NULL
concentracion_datos_original_ts_train_frame$tend <-concentracion_datos_original_ts_train_frame$tend.1.144.
concentracion_datos_original_ts_train_frame$tend.1.144. <- NULL

concentracion_datos_original_ts_train_frame
```



#### Se 'entrena' (en este caso, se define) al modelo lineal 'simple' con los datos de train 

```{r}
modelo_lineal_simple1_concentracion_datos_original <- glm (concentracion_datos_original_ts_train ~  rep_mes + tend, data=concentracion_datos_original_ts_train_frame, family = gaussian(link = identity))
modelo_lineal_simple1_concentracion_datos_original
```


Se obtiene el summary y plot del modelo lineal 'simple'

```{r}
summary(modelo_lineal_simple1_concentracion_datos_original)
```

Se puede observar que el modelo lineal simple resultante otorga valores estadisticamente significativos para el caso del intercepto y los "meses dummy" de: agost, dic, ene, feb, jul, jun , mar, may, nov , oct y sept con un valor de criterio de AIC de 146 



Vamos a proceder a plotear los residuos de mi modelo lineal 

```{r}
plot(modelo_lineal_simple1_concentracion_datos_original)
```

Se observa que los reiduos no tienen pendiente definida, tendencia o siguen un patron establecido, en general se observa que las gráficas del error son muy buenas 


#### Testeo del 'modelo lineal generalizado simple'  haciendo la predición de los datos de test 
```{r}


concentracion_datos_original_ts_test_frame <- data.frame(concentracion_datos_original_ts_test,rep_mes[145:154],tend[145:154])
head(concentracion_datos_original_ts_test_frame)
tail(concentracion_datos_original_ts_test_frame)
dim(concentracion_datos_original_ts_test_frame)

```



```{r}
concentracion_datos_original_ts_test_frame$rep_mes <-concentracion_datos_original_ts_test_frame$rep_mes.145.154.
concentracion_datos_original_ts_test_frame$rep_mes.145.154. <- NULL
concentracion_datos_original_ts_test_frame$tend <-concentracion_datos_original_ts_test_frame$tend.145.154.
concentracion_datos_original_ts_test_frame$tend.145.154. <- NULL

concentracion_datos_original_ts_test_frame
```




#### Calculo de predicción del 'modelo lineal generalizado simple' para los datos_original

```{r}
prediccion_modelo_lineal_simple1_concentracion_datos_original <- predict(modelo_lineal_simple1_concentracion_datos_original, newdata = concentracion_datos_original_ts_test_frame, se.fit = TRUE)


prediccion_modelo_lineal_simple1_concentracion_datos_original
```

```{r}
plot(prediccion_modelo_lineal_simple1_concentracion_datos_original$fit)
```





#### Calculo de errores del 'modelo lineal generalizado simple' para los datos_original


```{r}
accuracy(concentracion_datos_original_ts_test , prediccion_modelo_lineal_simple1_concentracion_datos_original$fit)
```

El MAPE del modelo lineal generalizado simple es MENOR que el de los métodos de SMA y WMA 



## Modelo SARIMA para Datos_original


#### Determinación de ACF & PACF para el caso de la colorado_data_original (SIN ESTACIONALIZAR)

```{r}
#install.packages("forecast")
library(forecast)

#par(mfrow=c(2,1))

acf_concentracion_datos_original_ts_sin_diferenciar <- Acf(concentracion_datos_original_ts, lag.max = 100, ci.type = "ma")
acf_concentracion_datos_original_ts_sin_diferenciar


pacf_datos_concentracion_original_ts_sin_diferenciar <- Pacf(concentracion_datos_original_ts, lag.max = 100, ci.type = "ma")
pacf_datos_concentracion_original_ts_sin_diferenciar

```

La función ACF de la serie colorado, nos muestra claramente una forma de decaimiento constante. Hagamos una prueba de estacionariedad 



#### Se lleva a cabo el tet de Dickey-Fulley (Ho: Serie no estacionaria) a la serie de datos_original <sin diferenciar>

```{r}
# Install and load the "urca" package
#install.packages("urca")
library(urca)

# Perform ADF test for stationarity
adf_test_datos_concentracion_original_ts <- ur.df(concentracion_datos_original_ts)

# Print the ADF test results
summary(adf_test_datos_concentracion_original_ts)

```
Se observa como el valor p de la prueva DF de estacioriedad fue menor a 0.05 por lo tanto se rechaza Ho que indicaba que la serie no era estacionaria y se acepta H1 





#### Vamos a diferenciar la serie de datos_orignal 'para hacerlo estacionaria'

```{r}
# Calculate the difference between consecutive elements
datos_concentracion_original_ts_diferencia1 <- diff(concentracion_datos_original_ts, differences = 1)

# Print the results
print(datos_concentracion_original_ts_diferencia1)

```



#### ACF & PACF para los datos_original con 1 diferencia  'estacionariazada'


```{r}
library(forecast)

#par(mfrow=c(2,1))

acf_datos_concentracion_original_ts_diferencia1 <- Acf(datos_concentracion_original_ts_diferencia1, lag.max = 100, ci.type = "ma")
acf_datos_concentracion_original_ts_diferencia1


pacf_datos_concentracion_original_ts_diferencia1 <- Pacf(datos_concentracion_original_ts_diferencia1, lag.max = 100, ci.type = "ma")
pacf_datos_concentracion_original_ts_diferencia1
```


La diferenciación de la seria ayudó a hacerla estacionaria y además a que se puediera disntinguir tanto los valores significativos para la parte MA del ARIMA, como la estacionalidad marcada cada 12 meses 



Con base en las gráficas de ACF y PACF se decide probar un modleo ARIMA (2,1,1)


#### Ajustae y predicicon de Demanda_electrico con modelo ARIMA 


```{r}
#install.packages("astsa")
library(astsa)

# Fit the SARIMA model
ARIMA_2_1_1_concentracion_datos_original <- astsa::sarima(concentracion_datos_original_ts_train , p=2, d=1, q=1)

# Print the model summary
summary(ARIMA_2_1_1_concentracion_datos_original)

```
En general se observa para el modelo ARIMA_2_1_1_concentracion_datos_original: 

Los errores estandarizados tienen un compartiempo aprorximadamente normal con media cero 
Los errores estandarizados no tiene una correlación signifcativa


#### Calculo de predicción del ARIMA para los datos_original

```{r}

# Get the predictors


pred_ARIMA_2_1_1_concentracion_datos_original <- sarima.for(concentracion_datos_original_ts_train, n.ahead=9,2,1,1)
pred_ARIMA_2_1_1_concentracion_datos_original


```




```{r}
pred_ARIMA_2_1_1_concentracion_datos_original$pred
```




#### Calculo de errores del modelo ARIMA para los datos_original

```{r}
accuracy(concentracion_datos_original_ts_test ,pred_ARIMA_2_1_1_concentracion_datos_original$pred )
```

El MAPE del modelo ARIMA 2,1,1 es muy bueno aún, aunque ligeramente mayor que el modelo lineal generalizado simple 



#### Ajustae y predicicon de Demanda_original con modelo SARIMA + PDQ S


#### Calculo de predicción del SARIMA PDQ S para los datos_original
```{r}
library(forecast)

# Fit the SARIMA model
SARIMA_concentracion_datos_original <- Arima(concentracion_datos_original_ts_train, order = c(2, 1, 1), seasonal = list(order = c(0, 1, 0), period = 12))

# Forecast for the next 12 periods
forecast_SARIMA_concentracion_datos_original <- forecast(SARIMA_concentracion_datos_original, h = 9)

# Print the forecasted values
print(forecast_SARIMA_concentracion_datos_original)
```

```{r}
plot(forecast_SARIMA_concentracion_datos_original)
```

Se observa claramente como los modelos SARIMA son capaces de tomar encuenta la estacionalidad de la data y aplicarla a los pronósticos 

```{r}
forecast_SARIMA_concentracion_datos_original$mean
```


#### Calculo de errores del Datos_orignal con SARIMA + PDQ S

```{r}
accuracy(concentracion_datos_original_ts_test ,forecast_SARIMA_concentracion_datos_original$mean )
```

El mejor MAPE obtenido al momento para el caso de colorado es con el modelos SARIMA(2,1,1)[0,1,0]con S= 12 así como el obtenido con el modelo lineal simple con variables dummy










