...
forecast_datos_original_ts_SMA<-forecast(datos_original_ts_SMA, h=12)
...
#### Calculo de predección con Weighted Moving Average (SMA) de datos_original

```{r}
library(forecast)

forecast_datos_original_ts_WMA<-forecast(datos_original_ts_WMA, h=12)
...
accuracy(datos_original_ts$VARIABLE[202:213] ,datos_original_ts_HW_forecast$mean )
```
...
```{r}
accuracy(datos_original_ts$VARIABLE[202:213] , prediccion_modelo_lineal_simple1_datos_original$fit)
```
...
```{r}
accuracy(datos_original_ts$VARIABLE[202:213] ,pred_ARIMA_1_2_1_datos_original$pred )
```
...
```{r}
accuracy(datos_original_ts$VARIABLE[202:213] ,forecast_SARIMA_PDQS_datos_original$mean )
```
