---
title: "Pronosticos_Class_16_Mayo_Aplicacion_Modelos_ARIMA_y_Lineales_a_mis_datos_demanda_residencial"
author: "Sergi"
date: "2023-02-27"
...
## Empezeos el análisis con los datos de Demanda_residencial

Primero importamos la data con el gas demandado en el sector residencial
```{r}

#URL DesktopDell
Demanda_residencial_importado<-read.csv("Demanda_residencial_2022_full1.csv", header= TRUE)
...
#### Convertimos los datos de Demanda_residencial a Serie de tiempo 

```{r}
Demanda_residencial_importado.ts<- ts(Demanda_residencial_importado$Demanded_Gas, frequency = 12, start =c(2005,1))
head(Demanda_residencial_importado.ts)
...
```{r}
plot(Demanda_residencial_importado.ts, col = "brown", main = "Demanda residencial 'original' ")
...
#### Separamos el df Demanda_residencial_importado en data sets train y test
...
Demanda_residencial_importado_train <- Demanda_residencial_importado[1:201,]
head(Demanda_residencial_importado_train)
tail(Demanda_residencial_importado_train)
dim(Demanda_residencial_importado_train)
...
Demanda_residencial_importado_test <- Demanda_residencial_importado[202:213,]
head(Demanda_residencial_importado_test)
dim(Demanda_residencial_importado_test)
...
Demanda_residencial_train_frame <- data.frame(Demanda_residencial_importado_train,rep_mes[1:201],tend[1:201])
head(Demanda_residencial_train_frame)
tail(Demanda_residencial_train_frame)
dim(Demanda_residencial_train_frame)

```

```{r}
Demanda_residencial_train_frame$rep_mes <-Demanda_residencial_train_frame$rep_mes.1.201
Demanda_residencial_train_frame$rep_mes.1.201. <- NULL
Demanda_residencial_train_frame$tend <-Demanda_residencial_train_frame$tend.1.201.
Demanda_residencial_train_frame$tend.1.201. <- NULL

Demanda_residencial_train_frame
```
...
```{r}
demanda_residencial_lineal_simple1 <- glm (Demanded_Gas ~ rep_mes + tend, data=Demanda_residencial_train_frame, family = gaussian())
demanda_residencial_lineal_simple1
```


```{r}
summary(demanda_residencial_lineal_simple1)

```


```{r}
plot(demanda_residencial_lineal_simple1)
```
...
Demanda_residencial_test_frame <- data.frame(Demanda_residencial_importado_test,rep_mes[202:213],tend[202:213])
head(Demanda_residencial_test_frame)
tail(Demanda_residencial_test_frame)
dim(Demanda_residencial_test_frame)

```

```{r}
Demanda_residencial_test_frame$rep_mes <-Demanda_residencial_test_frame$rep_mes.202.213.
Demanda_residencial_test_frame$rep_mes.202.213. <- NULL
Demanda_residencial_test_frame$tend <-Demanda_residencial_test_frame$tend.202.213.
Demanda_residencial_test_frame$tend.202.213. <- NULL

Demanda_residencial_test_frame
```
...
```{r}
prediccion_demanda_residencial_lineal_simple1 <- predict(demanda_residencial_lineal_simple1, newdata = Demanda_residencial_test_frame, se.fit = TRUE)


prediccion_demanda_residencial_lineal_simple1
```

```{r}
plot(prediccion_demanda_residencial_lineal_simple1$fit)
```
...
#### Calculo de errores del 'modelo lineal generalizado simple' para los datos de Demanda_residencial

```{r}
valores_calculados_demanda_residencial_lineal_simple1 <- prediccion_demanda_residencial_lineal_simple1$fit

# Calculate MAPE
mape_residencial_lm_simple1 <- mean(abs((Demanda_residencial_importado$Demanded_Gas[202:213] - valores_calculados_demanda_residencial_lineal_simple1) / Demanda_residencial_importado$Demanded_Gas[202:213])) * 100
cat("El MAPE del 'modelo lineal generalizado simple' para los datos de Demanda_residencial es:", round(mape_residencial_lm_simple1,2),"%\n")

# Calculate RMSE
rmse_residencial_lm_simple1 <- sqrt(mean((Demanda_residencial_importado$Demanded_Gas[202:213] - valores_calculados_demanda_residencial_lineal_simple1)^2))
cat("El RMSE 'modelo lineal generalizado simple' para los datos de Demanda_residencial es:", round(rmse_residencial_lm_simple1,2),"\n")
```
...
```{r}
valores_calculados_demanda_electrico_lineal_mas_datos_simple2 <- prediccion_demanda_electrico_lineal_mas_datos2$fit

# Calculate MAPE
mape_electrico_lm_mas_datos2 <- mean(abs((Demanda_electrico_importado$Demanded_Gas[202:213] - valores_calculados_demanda_electrico_lineal_mas_datos_simple2) / Demanda_electrico_importado$Demanded_Gas[202:213])) * 100
cat("El MAPE del 'modelo lineal generalizado con Poblacion para los datos de Demanda_electrico es:", round(mape_electrico_lm_mas_datos2,2),"%\n")

# Calculate RMSE
rmse_electrico_lm_mas_datos2 <- sqrt(mean((Demanda_electrico_importado$Demanded_Gas[202:213] - valores_calculados_demanda_electrico_lineal_mas_datos_simple2)^2))
cat("El RMSE de modelo lineal generalizado con Poblacion para los datos de Demanda_electrico es:", round(rmse_electrico_lm_mas_datos2,2),"\n")
```
...
#### Calculo de errores del lineal Demanda_electrico con SARIMA

```{r}
valores_calculados_demanda_electrico_lineal_SARIMA_1_1_1 <- pred_SARIMA_1_1_1_Demanda_electrico$pred

# Calculate MAPE
mape_electrico_SARIMA_1_1_1 <- mean(abs((Demanda_electrico_importado$Demanded_Gas[202:213] - valores_calculados_demanda_electrico_lineal_SARIMA_1_1_1) / Demanda_electrico_importado$Demanded_Gas[202:213])) * 100
cat("El MAPE del modelo SARIMA (1,1,1) para los datos de Demanda_electrico es:", round(mape_electrico_SARIMA_1_1_1,2),"%\n")

# Calculate RMSE
rmse_electrico_SARIMA_1_1_1 <- sqrt(mean((Demanda_electrico_importado$Demanded_Gas[202:213] - valores_calculados_demanda_electrico_lineal_SARIMA_1_1_1)^2))
cat("El RMSE de modelo modelo SARIMA (1,1,1)  para los datos de Demanda_electrico es:", round(rmse_electrico_SARIMA_1_1_1,2),"\n")
```
