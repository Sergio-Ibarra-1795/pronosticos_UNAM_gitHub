...
```{r}
knitr::include_graphics("Tipos_errores_clasificacion_imagen.png")
```
...
```{r}
knitr::include_graphics("Check_list_for_selecting_error_measure_depending_on_time_series_characteristics.PNG")
```


Aditional Notes: 
- RMSE (mean optimizer) and MAE (median optimizer) have"the same units" which are "the same units" as the yi variable 

- MSE have those units to the power of two

- RMSE and MAE are called "scale dependent" meassures as the units and magnitude are specific to the problem and context. An alternative approach is to use "scale invariant" measures such as the MAPE 

-A limitation of MAPE is that is inflated when the denominator is small relative to the absolute forecast error. It also penalises negative errors more than positve erros, so that MAPE can lead to selecting a model that tends to under forecast .
