---
title: "Pronosticos_Class_21_Marzo_Forecast_De_Modelo_Holt_Winters_y_descomposicion_series_2"
author: "Sergi"
date: "2023-02-27"
...
for (j in 1:12)
{
  ...
  bp=window(Demanda_electrico_importado.ts,start=c(2005,j), deltat=1)
  plot(bp, main=paste("mes",mes[j]),type="o",ylim=c(1000,4000))
...
for (j in 1:12) {
  ...
  bp <- window(Demanda_electrico_importado.ts, start=c(2005, j), deltat=1)
  tiempo <- 1:length(bp)  # Create a new tiempo vector with the length of bp
  
  plot(bp, main=paste("mes",mes[j]), type="o", ylim=c(0,5000))
  
  ajuste1 <- lm(bp ~ 1 + tiempo)  # Fit the linear model using bp and tiempo
  summary(ajuste1)
  plot(ajuste1)
  serie <- data.frame(tiempo, bp)
  rownames(serie) <- NULL  # Remove the row names from the data frame
...
If you have a seasonal time series, you can seasonally adjust the series by estimating the seasonal component(s), and subtracting it from the original time series. We can see below that time time series simply consists of the trend and random components.
...
plot.ts(Demanda_electrico_SINSeason, main = "Demanda electrico sin componente de seasonalidad", col="blue")
```
...
plot.ts(Demanda_electrico_SIN_Sea_NI_Trend, main = "Demanda electrico sin componente season ni componente trend", col="violet")
```
[Si se quiesera hace run pronóstico a partir de aqui debería "regresar la tendencia y la estacionalidad" para 
tener de nuevo los valores en la escala correcta]
